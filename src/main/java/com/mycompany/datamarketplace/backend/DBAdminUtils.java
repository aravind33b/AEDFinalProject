...
public class DBAdminUtils {
    ...
    public Boolean createCountryForGovt(String countryNameForGovt, String tableName) {
        Boolean isSuccess = false;
            if(tableName.equalsIgnoreCase("country")){
                ...
                Connection conn = createConnection();
                Statement statement = conn.createStatement();
                String query = "INSERT INTO `test_aed`.`"+tableName+"` (`community_id`,`community_name`) VALUES ('1','"+countryNameForGovt+"')";
                statement.executeUpdate(query);
                isSuccess = true;
            }
            catch(Exception e){
    ...
    public Community retrieveDevCommunityDetails(String communityName, String tableName) {
        Community company = null;
        try {
            String companyIdentity = "";
            ...
            while(rs.next()){
                ...
                if (companyIdentity.equalsIgnoreCase(communityName)){
                    company = new Community();
                    company.setCommunityName(rs.getString("community_name"));
                }
            }
